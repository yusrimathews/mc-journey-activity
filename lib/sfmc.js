...
SFMC_MIDs.forEach((mid, i) => {
  ...
  try {
    ...
    logger.error(`[sfmc.js] ${JSON.stringify(error)}`);
  }

  // Test connection
  restClients[SFMC_MIDs[i]].get({ uri: '/platform/v1/tokenContext' }, (error, response) => {
    if (error) {
      logger.error(`[sfmc.js] ${SFMC_MIDs[i]} : ${JSON.stringify(error)}`);
    } else {
      logger.debug(`[sfmc.js] ${SFMC_MIDs[i]} : ${JSON.stringify(response.body)}`);
    }
  });
});
  ...
  getJourney: (mid, definitionId) => new Promise((resolve, reject) => {
    ...
    restClients[`${mid}`].get(requestOptions, (error, response) => {
      if (error) {
        ...
        logger.debug(`[sfmc.js] ${JSON.stringify(response)}`);
